...
async function renderSettings() {
  ...
  // araç satırına (boş alana) dokun → kilometre güncelle
  $('set-vehicles').querySelectorAll('li[data-vid]').forEach(li =>
    li.addEventListener('click', async () => {
      const v = allV.find(x => x.id === +li.dataset.vid);
      if (!v) return;
